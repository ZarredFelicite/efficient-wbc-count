...
def salience(model, img, layers=None, hue=None):
    ...
    out = model(input)
    conf = int(torch.nn.functional.softmax(out, dim=1).max().item() * 100)
    pred = classes[out.squeeze(0).argmax().item()]
    print("{}% certainty the cell is a {}".format(conf, pred))
    ...
    if hue:
        # Distort colour
        ax1[num + 1].imshow(tF.to_pil_image(tF.adjust_hue(img, hue_factor=hue)))
        ax1[num + 1].axis("off")
        ax1[num + 1].set_title("Original")
        for idx, name, cam in zip(range(num), cam_extractor.target_names, cams):
            ax1[idx].imshow(cam.numpy())
            ax1[idx].axis("off")
            ax1[idx].set_title(name)
            overlay = overlay_mask(
                tF.to_pil_image(tF.adjust_hue(img, hue_factor=hue)),
                tF.to_pil_image(cam, mode="F"),
                alpha=0.5,
            )
            ax2[idx].imshow(overlay)
            ax2[idx].axis("off")
            ax2[idx].set_title(name)
        ax1[num].imshow(fused_cam.numpy())
        ax1[num].axis("off")
        ax1[num].set_title(" + ".join(cam_extractor.target_names))
        result = overlay_mask(
            tF.to_pil_image(tF.adjust_hue(img, hue_factor=hue)),
            tF.to_pil_image(fused_cam, mode="F"),
            alpha=0.5,
        )
        ax2[num].imshow(result)
        ax2[num].axis("off")
        ax2[num].set_title(" + ".join(cam_extractor.target_names))
        ...
        for idx, name, cam in zip(range(num), cam_extractor.target_names, cams):
            ...
            ax1[idx].set_title(name)
            overlay = overlay_mask(
                tF.to_pil_image(img), tF.to_pil_image(cam, mode="F"), alpha=0.5
            )
            ax2[idx].imshow(overlay)
            ax2[idx].axis("off")
            ax2[idx].set_title(name)
        ...
        ax1[num].set_title(" + ".join(cam_extractor.target_names))
        result = overlay_mask(
            tF.to_pil_image(img), tF.to_pil_image(fused_cam, mode="F"), alpha=0.5
        )
        ax2[num].imshow(result)
        ax2[num].axis("off")
        ax2[num].set_title(" + ".join(cam_extractor.target_names))
