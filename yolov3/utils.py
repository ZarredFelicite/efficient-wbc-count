...
def non_max_suppression(bboxes, iou_threshold, threshold, box_format="corners"):
    ...
    while bboxes:
        ...
        ######
        # Dont know why you would want to keep less confident predictions of other classes?
        # bboxes = [
        #     box
        #     for box in bboxes
...
def mean_average_precision(
    ...
    # print(
    #     "Average Precision:  Basophil: {} Eosinophil: {} Lymphocyte: {} Monocyte: {} Neutrophil: {}".format(
    #         average_precisions[0],
    #         average_precisions[1],
    #         average_precisions[2],
    #         average_precisions[3],
    #         average_precisions[4],
    #     )
    # )
...
def get_evaluation_bboxes(
    loader, model, iou_threshold, anchors, threshold, box_format="midpoint",
):
    ...
    for batch_idx, (x, labels) in enumerate(tqdm(loader)):
        ...
        with torch.no_grad():
            predictions = model(x)
        ####
        predictions = [cuda_tensor.cpu() for cuda_tensor in predictions]
...
def evaluation_box_class(
    loader, model, iou_threshold, anchors, threshold, device, box_format="midpoint",
):
    # make sure model is in eval before get bboxes
    model.eval()
    model.to(device)
    ...
    for batch_idx, (x, labels) in enumerate(tqdm(loader)):
        ...
        with torch.no_grad():
            out = model(x)
        ####
        predictions = [cuda_tensor.cpu() for cuda_tensor in out]
        y = labels
        batch_size = x.shape[0]
        bboxes = [[] for _ in range(batch_size)]
        for i in range(3):
    ...
    return all_pred_boxes, all_true_boxes, class_accuracy, no_obj_accuracy, obj_accuracy
...
def get_loaders(subset, path, batch_size):
...
def test_latency(model, device, batch_size, repetitions):
    model.eval()
    model.to(device)
    ...
    dummy_input = torch.randn(
        batch_size, 3, config.IMAGE_SIZE, config.IMAGE_SIZE, dtype=torch.float
    ).to(device)
    with torch.no_grad():
        # Warmup
        print("Warming Up")
